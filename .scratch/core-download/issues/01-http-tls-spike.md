...
- [ ] `moonbitlang/async` 加入依赖，`moon test` 在默认 wasm 与 native 目标均通过
- [ ] 本地 HTTP mock 返回 200，响应体字节与期望一致
- [ ] 对 `https://hf-mirror.com` 的 HTTPS GET 在 native 后端成功（或明确记录不可用及兜底方案）
- [ ] Range 请求头被 mock 记录；302 重定向被正确跟随
- [ ] `docs/design.md` 的 HTTP 层章节更新为最终选型结论
