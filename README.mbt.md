...
| `QUHONGYE-gif/modelhub/nn` | 张量与推理内核（GEMM/激活/归一化/注意力/KV cache） | ✅ | numpy golden 逐元素对照 |
| `QUHONGYE-gif/modelhub/transformers` | GPT-2 级模型推理（safetensors/前向/生成） | ✅ | numpy golden + 真实 tiny-random-gpt2 端到端生成 |
| `QUHONGYE-gif/modelhub/onnx` | 通用 ONNX 图解释执行器（11 算子） | ✅ | numpy golden + 官方 ONNX backend test 子集 |

已发布到 mooncakes：[`QUHONGYE-gif/modelhub`](https://mooncakes.io/docs/QUHONGYE-gif/modelhub)
（v0.1.0，安装：`moon add QUHONGYE-gif/modelhub`）。
